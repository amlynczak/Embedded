...
void ustawStatkiRand(Player *p){
	...
	/*}else{
		...
		p->boardPlayer.sqares[randX][randY+1] = 1;
		p->boardPlayer.sqares[randX][randY+2] = 1;
	}*/
...
bool shoot(float *tab, Player *player, ARM_DRIVER_USART * USARTdrv){
	...
	if(startFlag){
		writeString(196, 135, "shoot!", LCDBlack);
		while(1){
			touchpanelGetXY(x, y);
		...
		writeString(196, 135, "op. turn...", LCDBlack);
		while(1){
	...
	int x_b = result/10;
	int y_b = result - x_b*10;
	
	if(!startFlag){
			writeString(196, 135, "sprawdzam...", LCDBlack);
		if(player->boardPlayer.sqares[x_b][y_b] == 1){
			info[0] = 'h';
		...
		writeString(196, 135, "czekam...", LCDBlack);
		while(1){
		...
		if(info[0] == 'h'){
			player->boardOpponent.sqares[x_b][y_b] = 1;
			drawX(result);
		}else if(info[0] == 'm'){
			player->boardOpponent.sqares[x_b][y_b] = 0; // tutaj wartosc dla nietrafionego
			drawVoid(result);
			//startFlag = !startFlag;
		}
	}
